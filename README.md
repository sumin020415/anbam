...
| 👤 마이페이지 (네이버식 사이드바 허브 - 데스크탑 좌측 세로 / 모바일 상단 가로 메뉴) - 프로필 (아바타 업로드 + 닉네임 수정[가입과 동일 중복확인] + 가입일 + 활동 요약[제보·댓글·받은 좋아요·총 조회수, 추가 쿼리 0]) / 비밀번호 변경 (현재 비밀번호 재인증 → 새 비밀번호 2단계) / 내 활동 탭 (내 제보 / 내 댓글 / 좋아요한 글 이력) / 내 문의 (신고·문의 내역 + 상태 + 관리자 답변 + 문의 작성) / 문의내역 (관리자 전용 - 신고·문의 검토·답변) | ✅ |
...
| 🚨 제보글·댓글 신고 + 관리자 문의 (사유/카테고리 모달 + 타깃별 1인 1신고 + 일반 문의[대상 없음] - reports 테이블 3종 통합, check `num_nonnulls(post_id,comment_id) <= 1`) + 관리자 검토·답변 워크플로 (마이페이지 "내 문의" 상태/답변 확인 + 관리자 "문의내역" 메뉴 또는 `/admin/reports` [`profiles.is_admin` 가드]) | ✅ |
...
> 원본(Java/Spring + React)의 기능을 **100% 재현**한 뒤, 기존 기능을 개선하고 원본에 없던 기능을 추가했습니다.

### 기존 기능 - 그대로 재현 + 개선

| 기능 | 원본 (Spring + React) | 안밤 (Next.js + Supabase) |
|------|----------------------|--------------------------|
| 인증 | JWT / Spring Security 자체 구현 | Supabase Auth (HttpOnly 쿠키 + PKCE) + RLS, 보안 코드 약 80% 감소 |
| 비밀번호 재설정 | userId 확인 후 즉시 변경 | 이메일 링크 + PKCE 콜백 (탈취 방지) |
| 댓글 / 대댓글 | `parentId` 저장만, UI 미지원 | depth 0~2 들여쓰기 트리 UI 완성 |
| 이미지 | `imageUrl` 필드만 (업로드 UI 없음) | Supabase Storage 실제 업로드 (5MB + 미리보기 + orphan 정리) |
| 지도 클러스터링 | 단순 MarkerClusterer (1단) | 3단 줌 (자치구 / 동 / 개별) + RPC 카운트로 응답 6MB → 95KB |
| 위치 검색 | Kakao Places | Places + Geocoder 병렬 (명칭·도로명·지번 모두) |
| 분석 페이지 | 2탭 (제보 수 / 인구 대비 보안등) | 3탭 + 상시 사이드 패널 (안전 점수·인구밀도·1대당) + KST 시계열 |
| 마이페이지 | 내 글 / 내 댓글 조회·삭제 | 사이드바 허브로 확장 (아래 신규 기능 참조) |

### 안밤이 새로 추가한 기능 (원본에 없음)

- 🚨 **신고 + 관리자 워크플로** - 게시글·댓글 신고 (사유 5종) + 관리자 검토·답변 (`is_admin` RLS) + 일반 문의 (원본은 `Member.role` 필드만 있고 미사용)
- 👤 **마이페이지 확장** - 프로필 수정·아바타 업로드 / 비밀번호 변경 (현재 비번 재인증 2단계) / 활동 요약 / 좋아요한 글 이력 / 내 문의
- 👁 **조회수** (RPC + 세션당 1회 중복 가드) / 🙋 **작성자 아바타** (목록·상세·댓글)
- 🔎 **게시글 검색** (제목·본문 ilike) + **5종 정렬** (최신/좋아요/싫어요/조회/댓글)
- 🎚 **핀 필터 토글** + 검색·클릭 위치 마커 + "여기에 제보 작성" / 📍 상세 위치 지도
- 🧭 **자치구·동 좌표 보정** (Kakao Local REST API reverse geocoding, 누적 41,551 호출 / 100% 정확)
- ⚡ **성능 최적화** - RPC 카운트 fetch (63× 감소) + viewport bbox fetch (모바일 OOM 해결), Lighthouse 성능 95 / LCP 1.1s
- 📱 **반응형 모바일 UX** (햄버거 드로어 + 지도 서브바 + 가로형 카드) + 하단 푸터 / 스크롤 탑 / Floating 작성 버튼
- 🏗 **RN 이식 대비 설계** - 모든 DB 쿼리를 `lib/services/*.ts` 로 분리
...
│   ├── mypage/                       # MyPageView (사이드바 허브 - 데스크탑 좌측 세로 / 모바일 상단 가로 메뉴, 프로필[가입일+활동요약 dl]·비밀번호 변경·내 활동·내 문의·문의내역[admin]) / ProfileEditForm (닉네임 수정 + 가입식 중복확인) / AvatarUpload (프로필 사진 업로드 - avatars 버킷) / PasswordChangeFlow (2단계: 현재 비번 재인증 → 새 비번) / MyContentTabs (내 제보·내 댓글·좋아요 탭) / MyReports (내 문의 - 신고/문의 3종 배지 + 상태 + 대상 링크/내용 + 관리자 답변 + 문의 작성 버튼) / InquiryModal (관리자 문의 작성 - 카테고리 5종 + 내용)
...
│   └── services/                     # Supabase 쿼리/뮤테이션 (auth/profiles[getProfile / updateProfile / isNicknameTaken / isAdminUser]/posts[getPosts 통합 - 5종 sort latest·likes·dislikes·views·comments + 검색 ilike + escapeIlikeTerm + comments(count) join + reactions 별도 fetch + DB 정렬 가능/불가능 분기 + getPostsByAuthor / getLikedPosts (마이페이지 내 활동)]/comments[getComments + getCommentsByAuthor]/reactions/reports[createReport (postId\|commentId 분기) + createInquiry (대상 없는 일반 문의) + getMyReports / getAllReports / updateReport + REPORT_REASONS / INQUIRY_CATEGORIES / STATUS]/pins[**Phase 11 `getDistrictPinCounts` / `getDongPinCounts` RPC ⭐ (6MB → 95KB, 63× 감소)** + `DistrictPinCount` / `DongPinCount` 타입 + `getCctvPinsInBounds` / `getLampPinsInBounds` (개별 핀 모드 viewport bbox fetch - gte/lte lat·lng + BOUNDS_FETCH_CAP=2000) + `LatLngBounds` 타입]/storage[uploadPostImage / uploadAvatar / deletePostImage]/analytics[자치구 카운트 N 병렬 + KST 시계열])
...
docs/
└── schema.sql                        # Supabase 통합 SQL - 7테이블 + RLS + 트리거 + Storage 정책 + Data API GRANT + **§10 RPC (`get_district_pin_counts` / `get_dong_pin_counts`, Phase 11) + §11 RPC (`increment_post_view` - 조회수 +1, RLS 우회 security definer, 비로그인 포함 카운팅) + §12 reports (글·댓글 신고 + 관리자 문의 3종 통합, `post_id`/`comment_id` 최대 하나 `check num_nonnulls <= 1` [둘 다 null = 일반 문의] + admin_reply/status + `profiles.is_admin` + `is_admin()` security definer) ⭐** (재실행 안전, create or replace)
...
| 🛡 신고·문의 처리 | ✅ 제보글·댓글 신고 (사유 5종) + 관리자 직접 문의 (카테고리 5종) → 관리자 검토·답변 (마이페이지 "문의내역" 또는 `/admin/reports`, `profiles.is_admin` 가드) → 작성자 마이페이지 "내 문의" 확인. (향후) 신고 누적 시 자동 soft delete / 변경 이력(audit log) |
