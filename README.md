...
| ⚡ 메인 지도 RPC 성능 최적화 - `get_district_pin_counts` / `get_dong_pin_counts` PostgreSQL 함수로 GROUP BY COUNT 만 fetch (응답 **6 MB → 95 KB, 63× 감소**) + 줌 <3 진입 시 개별 핀 lazy fetch + transparent fallback + 우상단 "개별 핀 불러오는 중…" indicator | ✅ |
...
| ⚡ Phase 11 - 메인 지도 RPC 성능 최적화 | `docs/schema.sql §10` 의 `get_district_pin_counts` / `get_dong_pin_counts` PostgreSQL 함수 + `pins.ts` RPC services + `clusterByDistrict.ts` RPC 헬퍼 + `MapHome.tsx` lazy fetch + transparent fallback + 우상단 indicator + `(main)/page.tsx` 풀 fetch → RPC 전환. **응답 6 MB → 95 KB (63× 감소), 콜드 ~3~5초 / warm ~1~2초** | ✅ |
| 🚀 Phase 11 후속 성능 측정 + (선택) 추가 캐싱 | PageSpeed Insights 측정 (LCP/FCP) + Vercel Analytics 활성화 + (LCP > 3s 시 선택) `unstable_cache` 로 RPC 응답 5분 캐싱 / Edge Runtime (`runtime = 'edge'`) / 빌드 시 정적 JSON pre-generate | ⏳ |
